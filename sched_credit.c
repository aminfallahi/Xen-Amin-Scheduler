...
static void ubdate_bits(struct csched_vcpu *vc)
{
	vc->mask=vcpu_info(vc->vcpu,evtchn_upcall_mask);
	vc->pending=vcpu_info(vc->vcpu,evtchn_upcall_pending);
}
...
static inline void
__runq_insert2(unsigned int cpu, struct csched_vcpu *svc)
{
    const struct list_head * const runq = RUNQ(cpu);
    struct list_head *iter;
    ...
    const struct scheduler *ops, s_time_t now, bool_t tasklet_work_scheduled)
{
	...
	if (snext->pending==1 || snext->mask==1)
		tslice = MILLISECS(5);
